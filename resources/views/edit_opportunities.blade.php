    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
          <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
            <form method="POST" action="{{route('update_opportunity', $opportunity->id)}}">
                @csrf
                @method('PUT')

                <div class="mt-6 ml-4">
                    <label for="name" class="block font-medium text-lg"> Name:</label>
                    <input type="text" id="name" name="name" class="mt-1  w-3/4 rounded-md " value ="{{ $opportunity->name}}" required>
                </div>

                <div class="mt-6 ml-4">
                    <label for="type" class="block font-medium text-lg">Type:</label>
                    <input type="text" id="type" name="type" class="mt-1  w-3/4 rounded-md" value ="{{ $opportunity->type}}" required>
                </div>
                ...
                <div class="mt-6 ml-4">
                    ...
                    <textarea id="description" name="description" class="mt-1 w-3/4 rounded-md" rows="4" maxlength="500" required>{{ old('description', $opportunity->description)}}</textarea>
                ...
                <div class="mt-6 ml-4">
                    ...
                    <select id="amount" name="amount" class="mt-1 w-3/4 rounded-md" value ="{{ $opportunity->amount}} " required>
                ...
                <div class="mt-6 ml-4">
                    ...
                    <select id="stage" name="stage" class="mt-1 w-3/4 rounded-md" value ="{{ $opportunity->type}}" required>
                ...
                <div class="mt-6 ml-4">
                    ...
                    @else
                        <select id="account" name="account" class="mt-1 w-3/4 rounded-md" required>
                            <option value="" disabled selected>Link This Opportunity to an Account</option>
                            @foreach($accounts as $account)
                                <option value="{{ $account->id }}" {{ old('account', $opportunity->account_id) == $account->id ? 'selected' : '' }}>
                ...
                <div class=" mt-6 ml-4 flex items-center gap-4">
